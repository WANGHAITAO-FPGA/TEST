...
public:
    ...
    Wrapper_1(const char * name){
      ...
      signalAccess[5] = new CDataSignalAccess( top.tx_packet_req );
      signalAccess[6] = new CDataSignalAccess( top.tx_packet_trigger );
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\CODE_LIB\\TEST\\SPINAL_TEST\\.\\simWorkspace\\Gtx_Top/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\CODE_LIB\\TEST\\SPINAL_TEST\\.\\simWorkspace\\Gtx_Top/") + name + ".dat").c_str());
      #endif
    }
