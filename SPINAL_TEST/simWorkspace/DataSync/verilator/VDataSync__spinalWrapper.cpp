...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.clk_in );
      signalAccess[1] = new CDataSignalAccess( top.clk_out );
      signalAccess[2] = new CDataSignalAccess( top.reset );
      signalAccess[3] = new CDataSignalAccess( top.datain_valid );
      signalAccess[4] = new CDataSignalAccess( top.datain_ready );
      signalAccess[5] = new SDataSignalAccess( top.datain_payload );
      signalAccess[6] = new SDataSignalAccess( top.dataout );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\SCALAWORK\\TEST\\SPINAL_TEST\\.\\simWorkspace\\DataSync/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\SCALAWORK\\TEST\\SPINAL_TEST\\.\\simWorkspace\\DataSync/") + name + ".dat").c_str());
      #endif
    }
