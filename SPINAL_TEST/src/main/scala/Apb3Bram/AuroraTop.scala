...
class AuroraTop(addressApbWidth: Int, addressBRAMWidth: Int, dataWidth: Int, lookback:Boolean=false)  extends Component{
  ...
  val toparea = new ClockingArea(topClockDomain){
    ...
    ctrl.readAndWrite(tx_headtemp,0x03000,bitOffset = 0,documentation = "auroratx 发送帧头")

    ctrl.readAndWrite(tx_ctrl,0x03004,bitOffset = 0,documentation = "auroratx ctrl register")
  }
...
object AuroraTop{
  def main(args: Array[String]): Unit = {
    SimConfig.withWave.doSim(new AuroraTop(20,8,32,true)){dut=>
